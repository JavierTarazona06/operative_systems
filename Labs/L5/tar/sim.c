...
struct proc* next_proc()
{
    ...
    for (CurrentListIndex = 0; CurrentListIndex < 5; CurrentListIndex++)
    {
        if (ListCount(RunnableQueues[CurrentListIndex]) == 0)
        {
            continue;
        }

        for (p = ListFirst(RunnableQueues[CurrentListIndex]); p != NULL;
             p = ListNext(RunnableQueues[CurrentListIndex]))
        {
            if (p->state != RUNNABLE)
            {
                continue;
            ...
            p = ListRemove(RunnableQueues[CurrentListIndex]);
            break;
        }
...
void set_state(enum pstate state)
{
    ...
    for (p = ptable.procs; p < &ptable.procs[ptable.size]; p++)
    {
        ...
        if (state == RUNNABLE)
        {
            ...
            ListAppend(RunnableQueues[p->priority], p);
        }
...
int mainp(int argc, char** argv)
{
    ...
    /* init priority queues 0 - 4 */
    RunnableQueues = (LIST**) Malloc(sizeof(LIST*) * 5);
    for (i = 0; i < 5; i++)
    {
        RunnableQueues[i] = ListCreate();
        if (RunnableQueues[i] == NULL)
        {
            printf("NULL!");
        }
    }
    ...
    for (i = 0; (size_t) i < ptable.size; i++)
    {
        name = malloc(32);
        if (name == NULL)
        {
